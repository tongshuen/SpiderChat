...
- **帖子与评论**：公开论坛，帖子不端到端加密，明文存储于服务器，传输走 TransportEncryptor。支持两层评论结构（根评论按热度排序，回复按时间排序），墓碑删除不级联，编辑显示已编辑标记。
- **投票与热度**：支持 -1/0/1 投票，每用户每对象一票，改票自动回滚。热度公式综合投票、去重评论者数和时间衰减，后台每 5-10 分钟重算，提供"为什么这个排名"分解。新用户 24 小时内投票不计入热度。
- **服务器卡片与目录联邦**：每台服务器有签名卡片（名称、头像、标签、在线人数、热度等级等）。服务器目录联邦，目录条目带签名，接收方验证后缓存。帖子不联邦，跨服按 ID 直达寻址（本服→known_servers→DHT→失败明确错误，禁止广播回退）。
- **在线人数与分级限流**：在线 = TCP 活跃 + 60 秒内心跳，UUID 去重，管理员不计入。基于 load_ratio 的 8 级限流（消息减速→发帖减速→禁搜索→禁注册→禁 DHT→禁登录→踢人→全禁），hysteresis 0.05 防抖，优雅降级按序执行，管理员豁免。热度四级显示（活跃/中等/繁忙/拥挤），管理员可开关精确数字显示。
