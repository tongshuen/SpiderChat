...
Spider 由客户端（Client）和服务端（Server）两部分组成，同时共享一套加密协议与工具库。客户端提供图形界面（基于 customtkinter，回退至 tkinter），服务端则作为消息中继、离线存储、用户管理与 DHT 网络节点运行。客户端之间也可通过 P2P 直连模块（Direct Connect）在无需服务器的情况下通信，支持蓝牙、WiFi Direct、局域网及公网 IP 直连，还支持在Minecraft房间中直连和通过广播的互发现--像在局域网里一样。
...
4. 跨服务器联邦
   ...
   - 用户可在不同服务器甚至在Minecraft客户端之间互发消息、创建联邦群组，实现全球分布式聊天网络。
...
4. 我的世界（minecraft/）
   - 在 Minecraft 中使用精简版但仍然安全的Spdier！
